...
from legend import color_code, program_code
...
def readImage (imagePath: str):
    ...
    return height, width, c1, c2[y0:y1, x0:x1]
...
def printPlan (type: int, fileName: str, array: np.array) -> None:
    """
    ...
    for row in array:
        for col in row:
            if col == 0: text += ' ' # external area
                ...
                elif col == 2: text += ',' # interior walls
...
def printShapes (filename: str, shapes: list):
    text = ""
    count = 0
    for shape in shapes:
        if text: text += "\n{}".format(shape[0])
        else: text += str(shape[0])
        coors = shape[1:]

        # if count == 6: print(' ', coors)
        ...
        while i < len(coors) - 1:
            ...
            # if count == 6: print(i, temp, hanging)
        ...
        for each in coors:
            text += "|{},{},0".format(each[1], -each[0])
        count += 1
    output(filename, text)

def printImage (fileName: str, height: int, width: int, program: np.array, instance: np.array) -> None:
    """
...
def printJSON (fileName: str, plan) -> None:
    """
    format plan into dictionary format and write into a JSON file with the given file name
    """
